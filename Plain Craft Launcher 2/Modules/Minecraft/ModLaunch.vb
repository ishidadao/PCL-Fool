...
#Region "开始"
    ...
    Public Class McLaunchOptions
        ...
        ''' <summary>
        ''' 在多人启动前，根据玩家输入的服务器地址探测受信任的签名更新服务。
        ''' 默认值：False。单人启动不会进行服务器发现。
        ''' </summary>
        Public ManagedServerDiscoveryRequested As Boolean = False
        ''' <summary>
    ...
    Private Sub McLaunchStart(Loader As LoaderTask(Of McLaunchOptions, Object))
        ...
        Try
            ...
            Dim Loaders As New List(Of LoaderBase) From {
                New LoaderTask(Of Integer, Integer)("检查服务器内容", AddressOf FoolUpdateBeforeLaunch) With {.ProgressWeight = 20},
            ...
            Dim ClientFixLoader = Loaders.First(Function(Item) Item.Name = "补全文件")
            Dim ClientFixIndex = Loaders.IndexOf(ClientFixLoader)
            '内存优化
            Select Case Settings.Get(Of Integer)("VersionRamOptimize", Instance:=McInstanceSelected)
                Case 0 '全局
                    If Settings.Get(Of Boolean)("LaunchArgumentRam") Then '使用全局设置
                        CType(ClientFixLoader, LoaderCombo(Of String)).Block = False
                        Loaders.Insert(ClientFixIndex + 1, New LoaderTask(Of Integer, Integer)("内存优化", AddressOf McLaunchMemoryOptimize) With {.ProgressWeight = 30})
                    End If
                Case 1 '开启
                    CType(ClientFixLoader, LoaderCombo(Of String)).Block = False
                    Loaders.Insert(ClientFixIndex + 1, New LoaderTask(Of Integer, Integer)("内存优化", AddressOf McLaunchMemoryOptimize) With {.ProgressWeight = 30})
...
#Region "分方式登录模块"
    ...
    Private Sub McLoginMsStart(Data As LoaderTask(Of McLoginMs, McLoginResult))
        ...
        If Not OAuthClientId.Any Then
            Throw New Exception("$此开发测试版尚未配置 Microsoft OAuth Client ID，不能使用正版登录。" & vbCrLf &
                "请暂时选择离线登录并使用与原来完全相同的玩家名，或联系服务器管理员更新启动器。")
        End If
        Data.Progress = 0.05
        '检查是否已经登录完成
    ...
    Private Function MsLoginStep4(Tokens As (XSTSToken As String, UHS As String)) As (AccessToken As String, ExpiresAt As Integer)
        ...
        Catch ex As HttpRequestCodeException
            ...
            ElseIf ex.StatusCode = HttpStatusCode.Forbidden Then
                ...
                If ex.Response?.ContainsIgnoreCase("Invalid app registration") OrElse
                   ex.Response?.ContainsIgnoreCase("AzureAppNotPermitted") OrElse
                   ex.Response?.ContainsIgnoreCase("AppRegInfo") Then
                    Throw New Exception("$PCL-Fool 的 Microsoft 应用尚未获准访问 Minecraft Java 服务。" & vbCrLf &
                        "你的微软账号授权已经成功，这不是密码、电脑、代理或当前 IP 的问题。" & vbCrLf &
                        "请等待服务器管理员在 https://aka.ms/mce-reviewappid 提交的 App ID 审核通过；当前可暂时使用与原来完全相同名称的离线登录。")
                Else
                    Throw New Exception("$Minecraft 登录服务拒绝了本次凭据兑换（HTTP 403）。" & vbCrLf &
                        "这不代表微软网页授权失败。请将 PCL 日志交给服务器管理员，以区分 App ID 未审核、账号权限或网络拦截。")
                End If
...
NextInstance:
        ...
        If Server.Any Then
            ...
            Else
                '老版本
                Dim ServerHost = Server
                Dim ServerPort = "25565"
                If Server.StartsWithF("[") AndAlso Server.Contains("]") Then
                    '带方括号的 IPv6，可选端口
                    Dim ClosingBracket = Server.IndexOf("]"c)
                    ServerHost = Server.Substring(1, ClosingBracket - 1)
                    If Server.Length > ClosingBracket + 2 AndAlso Server(ClosingBracket + 1) = ":"c Then ServerPort = Server.Substring(ClosingBracket + 2)
                ElseIf Server.Count(Function(Character) Character = ":"c) = 1 Then
                    '普通主机名或 IPv4，带端口
                    Dim Colon = Server.LastIndexOf(":"c)
                    ServerHost = Server.Substring(0, Colon)
                    ServerPort = Server.Substring(Colon + 1)
                End If
                Args.Add("--server") : Args.Add(ServerHost)
                Args.Add("--port") : Args.Add(ServerPort)
