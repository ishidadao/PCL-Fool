...
#Region "更新"
    ...
    Friend Sub UpdateCheckByButton()
        MyMsgBox("这是《愚者》玩家定制版，不会使用 PCL 官方自更新覆盖自身。" & vbCrLf & vbCrLf &
                 "连接勾选了《此服务器提供自动更新》的服务器时，PCL 会从输入地址获取并验证签名发现文档和更新清单；校验或下载失败时会阻止启动。未勾选的服务器只进行普通直连。" & vbCrLf & vbCrLf &
                 "启动器程序本身的新版由服务器管理员另行发布。", "愚者更新说明")
    End Sub
