...
#Region "基础"
    ...
    Private Sub FormMain_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Sub()
            ...
            Telemetry("启动")
            '开源版本提示
            If VersionBranchMain = "OpenSource" AndAlso Not OAuthClientId.Any Then
                MyMsgBox("此开发测试版尚未配置 Microsoft OAuth Client ID，因此暂时不能使用正版登录。" & vbCrLf & vbCrLf &
                         "《愚者》服务器当前允许离线登录；请选择离线登录并使用与原来完全相同的玩家名。管理员配置自有 Client ID 并重新构建后，本提示会自动消失。",
                         "愚者启动器测试说明")
            End If
