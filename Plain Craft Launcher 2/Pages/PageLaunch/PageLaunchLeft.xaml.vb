...
#Region "切换大页面"
    ...
    Public Sub PageChangeToLaunching()
        ...
        LabLaunchingTitle.Text = If(CurrentLaunchOptions?.ManagedServerDiscoveryRequested,
            "正在检查并连接服务器",
...
#End Region
    ...
    Public Sub LaunchButtonClick() Handles BtnLaunch.Click
        ...
        If BtnLaunch.Text = "启动游戏" Then
            ...
            If TryBuildLaunchTarget(Options) Then McLaunchStart(Options)
...
#Region "启动目标"
    ...
    Private LaunchTargetUiReady As Boolean = False

    Private Sub InitializeLaunchTargetUi()
        LaunchTargetUiReady = False
        Dim History = GetLaunchServerHistory()
        ComboLaunchServer.ItemsSource = History.ToArray
        Dim LastServer = Settings.Get(Of String)("LaunchServerLast").Trim
        If LastServer = "" AndAlso History.Any Then LastServer = History.First
        ComboLaunchServer.Text = LastServer
        RefreshManagedOptionFromServer()
        If Settings.Get(Of Integer)("LaunchTargetMode") = 1 Then
            RadioLaunchMulti.SetChecked(True, False, False)
        Else
            RadioLaunchSingle.SetChecked(True, False, False)
        End If
        LaunchTargetUiReady = True
        RefreshLaunchTargetUi()
    End Sub

    Private Function GetLaunchServerHistory() As List(Of String)
        Dim Result As New List(Of String)
        For Each Entry In Settings.Get(Of String)("LaunchServerHistory").Split("¨")
            Dim Address = Entry.Trim
            If Address.Any AndAlso Not Result.Any(Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase)) Then Result.Add(Address)
        Next
        Return Result
    End Function

    Private Function GetManagedServerAddresses() As List(Of String)
        Dim Result As New List(Of String)
        For Each Entry In Settings.Get(Of String)("LaunchServerManagedAddresses").Split("¨")
            Dim Address = Entry.Trim
            If Address.Any AndAlso Not Result.Any(Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase)) Then Result.Add(Address)
        Next
        Return Result
    ...
    Private Sub RefreshManagedOptionFromServer()
        Dim Address = ComboLaunchServer.Text.Trim
        CheckLaunchServerManaged.SetChecked(GetManagedServerAddresses().Any(
            Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase)), False)
    End Sub

    Private Sub SaveLaunchServer(Address As String)
        Dim History = GetLaunchServerHistory()
        History.RemoveAll(Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase))
        History.Insert(0, Address)
        If History.Count > 20 Then History.RemoveRange(20, History.Count - 20)
        Settings.Set("LaunchServerHistory", String.Join("¨", History))
        Settings.Set("LaunchServerLast", Address)
        LaunchTargetUiReady = False
        ComboLaunchServer.ItemsSource = History.ToArray
        ComboLaunchServer.Text = Address
        LaunchTargetUiReady = True
    End Sub

    Private Sub RadioLaunchTarget_Change(sender As Object, raiseByMouse As Boolean) Handles RadioLaunchSingle.Check, RadioLaunchMulti.Check
        If Not LaunchTargetUiReady Then Return
        Settings.Set("LaunchTargetMode", If(RadioLaunchMulti.Checked, 1, 0))
        RefreshLaunchTargetUi()
    End Sub

    Private Sub ComboLaunchServer_TextChanged(sender As Object, e As TextChangedEventArgs) Handles ComboLaunchServer.TextChanged
        If Not LaunchTargetUiReady Then Return
        Settings.Set("LaunchServerLast", ComboLaunchServer.Text.Trim)
        LaunchTargetUiReady = False
        RefreshManagedOptionFromServer()
        LaunchTargetUiReady = True
        RefreshLaunchTargetDescription()
    End Sub

    Private Sub CheckLaunchServerManaged_Change(sender As Object, user As Boolean) Handles CheckLaunchServerManaged.Change
        If Not LaunchTargetUiReady OrElse Not user Then Return
        Dim Address = ComboLaunchServer.Text.Trim
        If Not Address.Any Then
            CheckLaunchServerManaged.SetChecked(False, False)
            Return
        End If
        Dim Managed = GetManagedServerAddresses()
        Managed.RemoveAll(Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase))
        If CheckLaunchServerManaged.Checked Then Managed.Insert(0, Address)
        Settings.Set("LaunchServerManagedAddresses", String.Join("¨", Managed))
        RefreshLaunchTargetDescription()
    End Sub

    Private Sub ComboLaunchServer_PreviewKeyDown(sender As Object, e As Input.KeyEventArgs) Handles ComboLaunchServer.PreviewKeyDown
    ...
    Private Sub BtnLaunchServerRemove_Click() Handles BtnLaunchServerRemove.Click
        Dim Address = ComboLaunchServer.Text.Trim
        Dim History = GetLaunchServerHistory()
        History.RemoveAll(Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase))
        Settings.Set("LaunchServerHistory", String.Join("¨", History))
        ...
        Managed.RemoveAll(Function(Saved) Saved.Equals(Address, StringComparison.OrdinalIgnoreCase))
        Settings.Set("LaunchServerManagedAddresses", String.Join("¨", Managed))
        Dim NextAddress = If(History.Any, History.First, "")
        Settings.Set("LaunchServerLast", NextAddress)
        LaunchTargetUiReady = False
        ComboLaunchServer.ItemsSource = History.ToArray
        ComboLaunchServer.Text = NextAddress
        RefreshManagedOptionFromServer()
        LaunchTargetUiReady = True
        RefreshLaunchTargetDescription()
        Hint(If(Address.Any, "已从服务器记录中移除。", "没有可移除的服务器地址。"), HintType.Blue)
    End Sub

    Private Sub RefreshLaunchTargetUi()
        PanServerAddress.Visibility = If(RadioLaunchMulti.Checked, Visibility.Visible, Visibility.Collapsed)
        CheckLaunchServerManaged.Visibility = PanServerAddress.Visibility
        RefreshLaunchTargetDescription()
    End Sub

    Private Sub RefreshLaunchTargetDescription()
        If BtnLaunchState = 3 AndAlso McInstanceSelected IsNot Nothing Then LabVersion.Text = GetLaunchTargetDescription()
    End Sub

    Private Function GetLaunchTargetDescription() As String
        If McInstanceSelected Is Nothing Then Return "未找到可用的游戏版本"
        If RadioLaunchMulti.Checked Then
            Dim Address = ComboLaunchServer.Text.Trim
            Return If(Address.Any, $"多人{If(CheckLaunchServerManaged.Checked, "（自动更新）", "")} · {Address}", "多人 · 请输入服务器地址")
        End If
        Return $"单人 · {McInstanceSelected.Name}"
    End Function

    Private Function TryBuildLaunchTarget(ByRef Options As McLaunchOptions) As Boolean
        Options = New McLaunchOptions
        If Not RadioLaunchMulti.Checked Then Return True

        Dim ServerAddress As String = Nothing
        Dim ErrorMessage As String = Nothing
        If Not TryNormalizeServerAddress(ComboLaunchServer.Text, ServerAddress, ErrorMessage) Then
            Hint(ErrorMessage, HintType.Red)
            Return False
        End If
        SaveLaunchServer(ServerAddress)

        Options.ServerIp = ServerAddress
        Options.ManagedServerDiscoveryRequested = CheckLaunchServerManaged.Checked
        Logger.Info($"启动目标：多人服务器 {ServerAddress}{If(CheckLaunchServerManaged.Checked, "（要求发现并验证签名更新）", "（普通直连，不检查更新）")}")
        Return True
    End Function

    Private Function TryNormalizeServerAddress(Input As String, ByRef Address As String, ByRef ErrorMessage As String) As Boolean
        Address = If(Input, "").Trim
        If Address.StartsWithF("minecraft://", True) Then Address = Address.Substring("minecraft://".Length).TrimEnd("/"c)
        If Not Address.Any Then
            ErrorMessage = "请输入服务器地址。"
            Return False
        End If
        If Address.Length > 255 OrElse Address.Contains("¨") OrElse Address.Any(Function(Character) Char.IsWhiteSpace(Character) OrElse Char.IsControl(Character)) OrElse
           Address.IndexOfAny(New Char() {"/"c, "\"c, "?"c, "#"c, ChrW(34)}) >= 0 Then
            ErrorMessage = "服务器地址格式不正确。请输入域名或 IP，可在末尾添加英文冒号和端口。"
            Return False
        End If

        Dim PortText As String = Nothing
        If Address.StartsWithF("[") Then
            Dim ClosingBracket = Address.IndexOf("]"c)
            If ClosingBracket <= 1 Then
                ErrorMessage = "IPv6 服务器地址格式不正确。"
                Return False
            End If
            Dim Suffix = Address.Substring(ClosingBracket + 1)
            If Suffix.Any Then
                If Not Suffix.StartsWithF(":") OrElse Suffix.Length = 1 Then
                    ErrorMessage = "IPv6 地址如需端口，请使用 [地址]:端口。"
                    Return False
                End If
                PortText = Suffix.Substring(1)
            End If
        ElseIf Address.Count(Function(Character) Character = ":"c) = 1 Then
            Dim Colon = Address.LastIndexOf(":"c)
            If Colon = 0 OrElse Colon = Address.Length - 1 Then
                ErrorMessage = "服务器地址的主机名或端口为空。"
                Return False
            End If
            PortText = Address.Substring(Colon + 1)
        End If
        If PortText IsNot Nothing Then
            Dim Port As Integer
            If Not Integer.TryParse(PortText, Port) OrElse Port < 1 OrElse Port > 65535 Then
                ErrorMessage = "服务器端口必须是 1 到 65535 之间的数字。"
                Return False
            End If
        End If
        Return True
    ...
    Public Sub RefreshButtonsUI() Handles BtnLaunch.Loaded
        ...
        Select Case CurrentState
            ...
            Case 3
                ...
                FrmLaunchLeft.BtnVersion.IsEnabled = True
                FrmLaunchLeft.BtnLaunch.IsEnabled = True
                FrmLaunchLeft.LabVersion.Text = GetLaunchTargetDescription()
    ...
    Public Sub LaunchingRefresh()
        Try
            ...
            LabLaunchingTitle.Text = If(IsLaunched OrElse McLaunchLoaderReal.State = LoadState.Finished,
                "已启动游戏",
                If(CurrentLaunchOptions?.ManagedServerDiscoveryRequested,
                    "正在检查并连接服务器",
                    If(CurrentLaunchOptions.SaveBatch Is Nothing, "正在启动游戏", "正在导出启动脚本")))
